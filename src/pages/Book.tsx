...
const Book = () => {
  ...
  return (
    <div className="mx-auto max-w-md px-5 pb-32 pt-6">
      ...
      <h1 className="mb-1 text-[32px] font-semibold leading-tight tracking-tight">
        ...
        <div className="space-y-3 animate-float-up">
          {services.map((s) => (
            <button
              ...
              className={cn(
                "press glass flex w-full items-center justify-between rounded-[22px] p-5 text-left transition-all",
      ...
      {step === 2 && (
        <div className="-mx-5 flex snap-x snap-mandatory gap-4 overflow-x-auto px-5 pb-4 no-scrollbar animate-float-up">
          {barbers.map((b) => (
            <button
              key={b.id}
              onClick={() => setBarber(b)}
          ...
          <div className="-mx-5 mb-5 flex gap-2 overflow-x-auto px-5 pb-2 no-scrollbar">
            {["Today", "Tomorrow", "Wed 24", "Thu 25", "Fri 26", "Sat 27"].map((d, i) => (
              <button
                key={d}
                className={cn(
                  "press glass min-w-[68px] rounded-2xl px-3 py-3 text-center transition",
            ...
            {timeSlots.map((t, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setSlot(t)}
                  className={cn(
                    "press glass rounded-2xl py-4 text-sm font-medium transition",
